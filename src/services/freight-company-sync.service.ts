...
import {
  FreightCompanySyncRequestDto,
  FreightCompanySyncRequestInput,
} from './dtos/freight-company-sync.dto';
import { FreightCompany } from 'src/entities/typeorm/freight-company.entity';
import { FREIGHT_COMPANY_QUERY } from 'src/common/constants/freight-company';

@Injectable()
export class FreightCompanySyncService {
  // QUERY SENSATTA
  private query = FREIGHT_COMPANY_QUERY;
  ...
  async getData() {
    const response =
      await this.oracleService.runQuery<FreightCompanySyncRequestInput>(
        this.query,
      );

    return response?.map((item) => new FreightCompanySyncRequestDto(item));
  }
  ...
  async syncWithStorage() {
    const data = await this.dataSource.manager.find(FreightCompany);

    const buffer = await FileUtils.toCsv(data);
    const s3Path = `sync-sensatta-snapshots/${EntitiesEnum.FREIGHT_COMPANY}-${DateUtils.getFileDate()}.csv`;
    ...
    await this.dataSource.manager.save(UtilsStorageSyncedFile, {
      ...
      entity: EntitiesEnum.FREIGHT_COMPANY,
      fileUrl: s3Path,
    });
