...
SELECT --SEXO
       ...
       ,CODIGO_ASSESSOR
       ,NOME_ASSESSOR
       ,ROUND(QUANTIDADE,0) QUANTIDADE_CABECAS
       ,CLASSIFICACAO  CLASSIFICACAO_GADO
       ,DECODE(PESO_MORTO,1,'PESO MORTO',0,'PESO VIVO')TIPO_PESAGEM
       ,PESO
       ,ROUND(ARROBA_CABECA,2) ARROBA_CABECA
       ,ROUND(ARROBA_CABECA * 15,2) KG_CABECA
       ,PRAZO
       ,VALOR_FRETE
       ...
       ,TOTAL VALOR_TOTAL
       ,ROUND((SELECT CASE 
           WHEN E.TIPO_ENTIDADE = 'F' THEN (TO_CHAR(VALOR_COMPRA) * 0.015)
             ELSE 0 END
         FROM SIGMA_VEN.ENTIDADE E
        WHERE E.ID_ENTIDADE = CODIGO_PECUARISTA),2)FUNRURAL
       ,VALOR_COMPRA_BRUTO - ROUND((SELECT CASE 
           WHEN E.TIPO_ENTIDADE = 'F' THEN (TO_CHAR(VALOR_COMPRA) * 0.015)
             ELSE 0 END
         FROM SIGMA_VEN.ENTIDADE E
        WHERE E.ID_ENTIDADE = CODIGO_PECUARISTA),2) VALOR_COMPRA_LIQUIDO
       ,(SELECT SUM(PG.PESO_BRUTO)
           FROM SIGMA_PEC.PESAGEM_GADO PG,
                SIGMA_PEC.RECEBIMENTO_GADO RG
          WHERE PG.ID_PESAGEM_GADO = RG.ID_PESAGEM_GADO)PESAGEM_BALANCA
...
SELECT DECODE( EG.SEXO ,'F','F�MEA','M','MACHO') SEXO,
  ...
  FROM SIGMA_PEC.ORDEM_COMPRA_GADO OC,
    ...
    WHERE PAR.CODIGO_EMPRESA = OC.CODIGO_EMPRESA
      ...
      --AND OC.ID_ORDEM_COMPRA_GADO = 15141
)TMP1
)tmp
) XY
) X
